...
Open the URL Vite prints. The **world map** is home. Click water to load that kilometre (GEBCO floor, a mean current, every catalogued animal whose range and thermal niche cover the cell). Land is not a cell. **Cells** picks a named kilometre: the 10 km catalog tank (whole catalog, stepped shelves, 2000 m) plus biomes from the coupling figure. Gap tiles still open the pelagic water at that site — they do not invent coral or a vent. Polar ice and the demersal bed field are coupled. Each named cell starts with the fauna that range would put there; names in the picker add or remove that biome’s usual cast.
...
| **Benthos** | Carbon on the seafloor from sinking detritus, plus microphytobenthos and infauna. Cod graze the living bed. |
| **Infauna** | Living seafloor density (worms, amphipods, as a field). Grows from detrital carbon and microphytobenthos. Cod graze it. Not named crabs. |
| **Microphytobenthos** | Light-driven algae on sunlit sand. Extra bed production where PAR hits the floor. The shelf analog of ice algae. |
...
**Clock** — one on-screen day is 480 s (~8 min). Twelve sim days per year. Live clock on by default (`L`). Hour slider pauses it. Storm (`T`) deepens the mixed layer, shoals the nutricline, and raises current speed. Night / dawn / day / dusk are a look (`src/simulation/day.js`) that also drives surface PAR and DVM. At high latitude the look follows solar elevation: polar night stays dark at noon; midnight sun stays lit at hour 0. **Named Cells** stamp a productive-season day-of-year (`observeDayIndex`): the Antarctic slope opens ~15 Jan, the high Arctic ~21 Jun, mid-latitudes stay 180. Map roam keeps the calendar you already have.
...
| **Temperature** `sampleTemp` | SST = latitude climatology + seasonal cycle + `sstAnomaly`. High-latitude amplitude is small so an Antarctic January cell stays polar, not 11 °C. Mixed layer well-mixed; below it a fall toward deep water. Winter mixes deeper; **storms mix deeper still** (`thermoY`). Q10 (`columnQ10`, `productionQ10`) scales NPZD, school metabolism/graze, and vehicle drain. Catalog `temp.min` / `temp.max` gates presence. |
...
| **NPZD** `plankton.js` | Separable 3D: \(C(x,y,z)=\mathrm{Patch}(x,z)\times\mathrm{Column}(y)\). 128×128 typed arrays `n`, `p`, `z`, `d` — not a 128³ grid. Column shape: P in the photic / DCM, Z on a DVM **plus an under-ice film when the cell holds ice**, N a nutricline that **shoals under climate upwell and storms**, D sinks. **Ice algae** adds P in the top ~10 m and a 2D production term when the cell holds ice. `sampleAt` / `grazeAt` return 0 below the local seafloor. Production uses PAR × P profile; Z grazing uses P–Z column coincidence and slows in a dark or cold column (same `productionQ10` as P). Type II half-saturation. Grazer spawn clips to `carryingCapacity` (bloom × light² × Q10). Carcasses and excretion return mass to `n` and `d`. The bed is three 2D stores on the same patch: detrital carbon, microphytobenthos, infauna. |
| **Benthos** | Column-bottom detritus flux onto a 2D seafloor store. **Microphytobenthos** grows where `samplePAR` at the local floor is photic — extra bed production, the shelf analog of ice algae. **Infauna** is a living 2D density that Type II-grazes both stores. Remineralises to N. Cod `grazeBenthos` takes infauna, not raw carbon. Present in every wet cell. The abyss is rain-only. HUD reads carbon, infauna, and microphyto. |
...
**School** (`src/simulation/school.js`) — one hashed grid. Grazers share a bloom-capped budget, split by catalog `share`. Bite-only taxa (`diet: "bite"`) take a prey-capped slice of the same grid (`piscivorePreyRatio`, default 22 forage per piscivore). `diet: "both"` (mackerel) stays on the grazer cap and still neighbour-bites. Neighbour-walk bite radius is at least one hashed-grid cell so a hunter in the prey band can land a meal. Social modes: `polarized` (herring pancake, skipjack), `loose` (flying fish, saury, mahi, sailfish), `scatter` (lanternfish, krill, school squid, Humboldt, barracuda, cod). Type II graze on `z` unless `grazeOn: "p"` (krill, menhaden) or bite-only. Hungry shoals rise or sink toward food or a prey centroid — including `habitat: "benthic"` (cod, toothfish), who leave the bed when hungry and sit on it when satiated. After a bloom clip, survivors pack into live shoals so a handful of lanternfish are still a hunt target, not eight empty scatter slots. `iceAssociated` (polar cod, krill, silverfish) shoals DVM toward the ice–water film when the cell holds ice. Fear: surface taxa steer up (still in water); lanternfish, Humboldt, and sand lance steer down. School squid and Humboldt pulse–coast on the grid velocity, then hang on `sampleFlow`. Sex is a size tint (females slightly larger). Recruits when the mixed budget can carry them; starve-cull recycles.

**Vehicles** (`src/simulation/shark.js`) — rares and air-breathers: sharks, whale shark, bluefin, mysticetes, odontocetes, giant squid. Raise `count` for a pod; do not put a shelf gadid or a jumbo-squid pack here. Gaits `burst` (glide between tail kicks), `ram` (must keep swimming), `jet` (pulse–coast, hang on the current). Swim: lateral tail, body wave, thunniform, vertical fluke, jet. Diets: `bite` school / `huntTaxa` / `huntKinds`, `filter` (`plankton.grazeAt` on `z`), `both` (minke, humpback). Orca `huntTaxa` is forage (clupeids, capelin, krill, squid), not the handful of slope gadids. A striking vehicle uses `lungeBiteRadius`, floored at one hashed-grid cell. Sighted hunters (`sense: "sight"`, the default) scale detect, fear, and bite with `visualRange` / `samplePAR`; lanternfish photophores restore a fraction in the DSL. Sperm whale and orca `sense: "echo"` — darkness is not a starve; strike search is length-scaled metres, not a 7 m visual bubble. Hungry DVM vehicles (giant squid) leave the day band toward live prey; satiated animals sit on the clock. Energy drain, meals restore, starve after `starveDays`, carcass recycles. Females pup on a year-timer when energy and a mate are in range. Pilot mode drives the lead blue shark (`P`).
...
- School grazer ceiling = `min(slider cap, bloom mean × photic production × photicLight × Q10)`.
- School piscivore ceiling = live forage / `piscivorePreyRatio` (default 22), split by `share` on the same grid. A tight bloom still leaves at least one agent for the highest-share biters when prey exists.
- Outside the catalog tank, vehicle headcount scales with live forage so 18 dolphins do not spawn on 14 clipped anchovy.
...
| cod | demersal · scatter | named shelf forage + infauna | bed · 600 m | `habitat: "benthic"`. Hungry shoals leave the bed toward prey; satiated fish graze the infauna field. Shelf only (dropped if floor ≲ −650 m). |
| toothfish | slope · scatter | silverfish | bed · 2000 m | Antarctic slope. Hungry climb toward silverfish. Not the 650 m gate. |
| humboldtsquid | cephalopod predator · scatter | anchovy, sardine, mackerel, lanternfish, jack mackerel | −80 / OMZ core · 1200 m | Jet on the grid. Hungry packs leave the OMZ toward prey. `needOmz`. Enlarges `gridMinY`. Sperm `huntTaxa`. |
...
| spermwhale | burst · fluke | `huntTaxa` market squid, Illex, lanternfish, Humboldt; `huntKinds` giant squid | 2000 m · ~700 m | Air-breather. `sense: echo` (length-scaled detect). `|lat| < 55`. Left spout. No free calories. |
...
The kilometre we load today is a **pelagic cell**. Polar ice is a field on that cell: concentration, under-ice PAR, ice-algal P, cryopelagic DVM. The seafloor is a living bed field: rain, microphytobenthos, infauna density, cod graze. Two other systems are first-class ocean, not later colour, so a coral, a vent, or an estuary is not buried under “named benthos.”
...
Soft sediment, rocky shelf, kelp, slope, seamount, abyss, hadal, seeps, and vents. Carbon rains onto the bed; sunlit sand grows microphytobenthos; infauna is a density field; cod graze that film. Named animals of the bed are still not agents.

| Gap | Why it matters |
| --- | --- |
| Named infauna and epifauna as agents | Polychaetes, amphipods, bivalves, nematodes, meiofauna — the field is density, not those taxa |
| Crabs, lobsters, shrimp, amphipod scavengers | Cod’s “crabs” are the infauna field |
