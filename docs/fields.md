...
3D concentration is separable: \(C(x,y,z) = \mathrm{Patch}(x,z)\times\mathrm{Column}(y)\). Horizontal mass is 128×128 (`n`, `p`, `z`, `d`). The column is a shared shape, not a second budget and not a 128³ grid: P follows the photic / DCM, Z a DVM, N a nutricline that shoals under climate upwell and storms, D sinks. `sampleAt` / `grazeAt` apply the product and return 0 below the local seafloor. Production uses PAR weighted by the P profile and an absolute `productionQ10` (tropics faster; polar P, Z graze, and Z mortality all share that clock so copepods do not outrun a cold bloom). Z grazing uses P–Z column coincidence; detritus export to `benthos` scales with the column bottom. Sunlit floors grow **microphytobenthos** (`bedP`) from PAR at the local seafloor. **Infauna** Type II-grazes carbon and microphyto. Cod `grazeBenthos` takes infauna, not raw carbon. `carryingCapacity` scales the school grazer cap with bloom mean, `prodIndex`, live `photicLight`, and `productionQ10`. Z mortality is low enough that a thin bloom still grows zooplankton — it does not require P ≈ 0.7 — and gains a density-dependent term so a seeded pack does not eat the bloom in a day. When the cell holds ice, the Z column keeps a film lobe so ice-associated grazers are not sitting in an empty metre.

`overlap(look, y, layer)` is the 0–1 encounter weight. `grazeBenthos` is the demersal bite on infauna. Ice algae (`iceAlgaeWant`) adds P in the top ~10 m when the cell holds ice. The renderer stacks slices on the live column bins so P is green in the photic and Z sparkles on the DVM.
