...
export const FAUNA = {
  ...
  tuna: {
    ...
    program:
      "Polarized hashed-grid tuna (`diet: bite`). Charges through a live forage school — including depth — then neighbour-walk bites in the photic-to-upper-mesopelagic band. Share of the prey-capped school slice, not a Reynolds handful. o2Min 2.4 ml/L — they will not follow lanternfish or Humboldt into the OMZ. Recruit from the mixed school budget when energy holds. Circumtropical oceanic hulls; the Mediterranean is empty.",
    missing: [
  ...
  humpback: {
    ...
    program:
      "Burst vehicle, fluke swim, huge bite radius, long pecs, ventral grooves on the throat. Slow turn, little bank; pecs beat. Hunts school fish and krill. Air-breather: hangs level at the surface and blows two columns, then a foraging dive toward prey or typical forage ~60 m, clamped by min(500 m, this cell's floor). Typical 10 min dive / 2.5 min blow series, mapped 5× onto wall-clock plus a commute pad. Leaves for the surface before the tank is empty (nose-up swim; tank still drains). Empty tank underwater is drowning. Recovery only counts at the air. Feeding hulls occupy local summer; tropical hulls occupy local winter. Wikipedia Cypron raster clips cells outside that map. Needs school prey in the cell.",
    missing: [
  ...
  spermwhale: {
    ...
    program:
      "Burst vehicle, fluke swim, block head (spermaceti organ, underslung jaw, left blowhole, knuckles to the fluke). The head stays stiff; only the tailstock waves. Slow turn, little bank. When two are in the cell they travel as one unit (`pods: 1`). Air-breather: hangs level at the surface and blows a single forward-left spout, then a foraging dive toward live squid/lanternfish or typical forage ~700 m — not a commute to 2000 m. maxDepth 2000 m is the clamp; in a 1500 m cell the floor wins first. Prefers market squid, Illex, lanternfish, and Humboldt squid (huntTaxa). Bites giant squid on a closing dive (`huntKinds`) — echo acquires across the cell (~120 m), then the mouth has to finish. A full oceanic cell holds a handful of giant squid, not a 1-vs-1 lottery. Typical 45 min forage / 8 min blow series, mapped 15× onto wall-clock (~6 min dive plus a commute pad) so it still finishes inside the 16 min day. Leaves for the surface before the tank is empty (nose-up swim; tank still drains). Empty tank underwater is drowning. Recovery only counts at the air. Does not bite herring. Missing named prey still produces the dive; it does not get free calories. sense is echo: PAR does not shrink this whale's detect or fear.",
    missing: [
  ...
  orca: {
    ...
    program:
      "Ram vehicle, authored orca glTF (taller dorsal on males; white eye patch and belly). One matriline (`pods: 1`, count 5–8): spawn as a unit, travel in formation, dive and blow on the leader's breath clock. Large fear radius. Bites school fish. Air-breather: hangs level at the surface and blows a short puff, then a foraging dive toward the school or typical forage ~90 m, clamped by min(800 m, this cell's floor). Typical 6 min dive / ~1.2 min blow series, mapped 5× onto wall-clock plus a commute pad. Leaves for the surface before the tank is empty (nose-up swim; tank still drains). Empty tank underwater is drowning. Recovery only counts at the air. This is the fish-eating programme. Banks in the turn like a dolphin, not a rorqual. sense is echo: PAR does not shrink detect or fear.",
    missing: [
  ...
  commondolphin: {
    ...
    program:
      "Ram vehicle, authored dolphin glTF, smaller than orca. Banks in the turn. Split into 1–3 pods (`podSize` ~10) as count scales (18–28); each pod travels, hunts, and blows together, and pods keep a gap so they do not merge. huntTaxa is flying fish, sardinella, anchovy, and sardine — the surface loop mahi also uses. Air-breather: hangs level at the surface and blows a small puff, then a foraging dive toward that prey or typical forage ~18 m, clamped by min(300 m, this cell's floor). Typical 2.5 min dive / ~40 s blow series, mapped 5× onto wall-clock plus a commute pad. Leaves for the surface before the tank is empty (nose-up swim; tank still drains). Empty tank underwater is drowning. Recovery only counts at the air. Geographic occupancy is the Wikipedia IUCN 2021 raster (warm-temperate bands, not a filled tropical gyre).",
    missing: [
  ...
  mahi: {
    ...
    program:
      "Loose hashed-grid hunter in the surface band (`diet: bite`). Charges through a live forage school, including depth, then neighbour-walk bites on school fish. Prey-capped share, not a vehicle pair. Circumtropical oceanic hulls plus the Mediterranean.",
    missing: [
  ...
  yellowfin: {
    ...
    program:
      "Polarized hashed-grid tuna (`diet: bite`). Charges through a live forage school, including depth, then neighbour-walk bites on school fish. Prey-capped share. o2Min 2 ml/L so the −500 m record is an oxygen ceiling, not a commute. Circumtropical oceanic hulls; the Mediterranean is empty.",
    missing: [
  ...
  sailfish: {
    ...
    program:
      "Loose hashed-grid billfish (`diet: bite`). Charges through a live forage school, including depth. Circumtropical oceanic hulls with school prey. Neighbour-walk bites restore energy on the prey-capped slice.",
    missing: [
  ...
  giantsquid: {
    ...
    about:
      "The deep oceanic squid sperm whales actually hunt. Mantle to about 2 m; total length often 8–13 m. Worldwide in ice-free deep water, typically 300–1000 m, recorded near 1200 m. Not a Humboldt jumbo: slower, deeper, and not tied to the East Pacific OMZ. A full oceanic cell holds a handful of vehicles (count 5 / max 8), not a single duel.",
    program:
      "Jet vehicle on its own DVM (night ~−420 m, day ~−850 m, max −1200 m). Longer hang between pulses than Humboldt. Bites lanternfish, market squid, and Illex (huntTaxa). In low PAR, lanternfish photophores restore a fraction of detect range. Absent on shelves shallower than about 350 m and under pack ice. Sperm whales lock this vehicle by echo and close (`huntKinds`). Starves if named prey is missing — no free calories.",
    missing: [
