...

{
  ...
  const cafeMar = presenceAt(28, -132, { floorY: -4000, dayOfYear: 60, trophic: false });
  const cafeJun = presenceAt(28, -132, { floorY: -4000, dayOfYear: 180, trophic: false });
  assert(cafeMar.greatwhite > 0, "White Shark Café is in the IUCN great-white band");
  assert(cafeJun.greatwhite > 0, "IUCN paints the Café yellow year-round — June should not empty it");
  const capeJan = presenceAt(41.7, -70, { floorY: -80, dayOfYear: 15, trophic: false });
  const pelagicGw = presenceAt(12, -30, { floorY: -4000, dayOfYear: 180, trophic: false });
  const medGw = presenceAt(36, 18, { floorY: -200, dayOfYear: 180, trophic: false });
  const nzGw = presenceAt(-41.3, 174.8, { floorY: -200, dayOfYear: 180, trophic: false });
  const npacGyre = presenceAt(28, -150, { floorY: -4000, dayOfYear: 180, trophic: false });
  const spacGyre = presenceAt(-20, -120, { floorY: -4000, dayOfYear: 180, trophic: false });
  assert(capeJan.greatwhite > 0, "Cape Cod is in the IUCN great-white band");
  assert(pelagicGw.greatwhite > 0 && pelagicGw.greatwhite < 0.8, "tropical Atlantic is IUCN possibly-extant, not empty");
  assert(medGw.greatwhite > pelagicGw.greatwhite, "Mediterranean is IUCN extant resident");
  assert(nzGw.greatwhite > pelagicGw.greatwhite, "New Zealand is IUCN extant resident");
  assert(npacGyre.greatwhite > pelagicGw.greatwhite, "North Pacific gyre is IUCN extant");
  assert(spacGyre.greatwhite > pelagicGw.greatwhite, "South Pacific gyre is IUCN extant");
  assert(presenceAt(21.2, -157.8, { floorY: -2000, dayOfYear: 180, trophic: false }).greatwhite > 0, "Hawaii is in the IUCN great-white band");
  assert(!(presenceAt(56, 3.2, { floorY: -71, dayOfYear: 180, trophic: false }).greatwhite > 0), "North Sea is north of the great-white band");
  assert(vehicleCountFor("greatwhite", pelagicGw.greatwhite) < vehicleCountFor("greatwhite", medGw.greatwhite), "great-white count should be lower in possibly-extant water than in extant resident water");
  ...
  assert(presenceAt(36, -125, { floorY: -4000, ...wiki }).shark > 0, "California Current is blue-shark water on the Wikipedia map");
  assert(!(presenceAt(80, 0, { floorY: -2000, ...wiki }).shark > 0), "high Arctic is north of the blue-shark Wikipedia map");
